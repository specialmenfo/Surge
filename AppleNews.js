...
if (url.indexOf(path1) != -1) {
    ...
    configs.deviceInfo.preferredLanguages = [“zh-Hant", "zh-CN", "zh-Hans", "en-US"];
    if ([‘HK', 'MO', 'US', 'CA', 'UK', 'AU'].some(_ => _ != configs.deviceInfo.countryCode)) configs.deviceInfo.countryCode = "US";
    ...
    if (async.data.session.mobileData) {
        async.data.session.mobileData.countryCode = "455";
        async.data.session.mobileData.carrier = "China Telecom";
        async.data.session.mobileData.networkCode = "02";
    };
