...
if (typeof $.getdata("GetSomeFries") != "undefined") {
	...
	$.Netflix = {
		...
		"config":{
			"allowWidevinePlayback": true, // 允許Widevine DRM回放
			"airPlayDisabledEnabledOnBuild": "50.0.0", // 開始禁用airPlay的版本
			"preferRichWebVTTOverImageBasedSubtitle": true, // 偏好使用RichWebVTT字幕多於圖片字幕
			"requestRichWebVTTAsExperimental": true, //試驗性請求RichWebVTT字幕
			"reuseAVPlayerEnabledOnBuild": "0", // 重新開始啓用AVPlayer的版本
			"nfplayerReduxEnabledOnBuild": "50.0.0", // 開始啓用nfplayerRedux的版本
		},
		"ctx":{
			"region": "", // 當前IP所屬地區
			//"monotonic": true, // 函數？
			"device": "", // 當前使用設備
			//"isolate_pool": true, // 隔離池？
			//"iter": 0,
			//"abtests":55, // AB測試
			//"ts":1642392069933, // timestamp？
			"ip": "",
			"hasUser": false // 當前IP是否有用戶
		}
	}	
};
...
if (url.search(path1) != -1) {
	...
	if (content.value?.geolocation) $.msg($.name, `已修改配置文件鏈接`, `策略: ${content.value.geolocation.policy}, 國家: ${content.value.geolocation.country}`)
...
//Check IP Status
	...
	if (content.ctx) $.msg($.name, `已修改IP檢測鏈接`, `地區: ${content.ctx?.region}, 設備: ${content.ctx?.device}, IP: ${content.ctx?.ip}, 已有用戶: ${content.ctx?.hasUser}`)
	body = JSON.stringify(content);
	$.done({ body });
}
