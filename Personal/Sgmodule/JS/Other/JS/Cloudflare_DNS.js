...
async function DDNS(zone, dns_records) {
	try {
		$.log(`開始更新${dns_records.type}類型記錄`);
		//Step 3
		...
		$.log(`${newRecord.name}上的${newRecord.type}記錄${newRecord.content}更新完成`, '');
	} catch (e) {
		$.logErr(e);
	} finally {
		return $.log(`${DDNS.name}完成`, `名稱:${dns_records.name}`, `type:${dns_records.type}`, `content:${dns_records.content}`, '');
	}
};
...
async function Verify(Mode, Content) {
	...
	if (Mode == "Token") {
		...
		$.logErr('無可用授權方式', `Mode=${Mode}`, `Content=${Content}`, '');
		$.done();
	}
}
...
async function checkZoneInfo(zone) {
	...
	if (zone.id && zone.name) {
		$.log(`有區域ID${zone.id}和區域名稱${zone.name}, 繼續`, '');
		newZone = zone;
	} else if (zone.id) {
		$.log(`有區域ID${zone.id}, 繼續`, '');
		newZone = await getZone(zone);
	} else if (zone.name) {
		$.log(`有區域名稱${zone.name}, 繼續`, '');
		newZone = await listZones(zone);	
	} else {
		$.logErr('未提供記錄ID和名稱, 終止', '');
		$.done();
	}
	$.log(`區域查詢結果:`, `ID:${newZone.id}`, `名稱:${newZone.name}`, `狀態:${newZone.status}`, `僅DNS服務:${newZone.paused}`, `類型:${newZone.type}`, `開發者模式:${newZone.development_mode}`, `名稱服務器:${newZone.name_servers}`, `原始名稱服務器:${newZone.original_name_servers}`, '');
...
async function checkRecordContent(dns_records) {
	if (dns_records.type) {
		$.log(`有類型${dns_records.type}, 繼續`, '');
		dns_records.type = dns_records.type;
		if (dns_records.content) {
			$.log(`有內容${dns_records.content}, 跳過`, '');
			...
			if (dns_records.type == 'A') dns_records.content = await getPublicIP(4);
				...
				$.log(`類型${dns_records.type}, 無內容，也不需要獲取外部IP,中止`, '');
				$.done();
			}
		}
	} else {
		$.log(`無類型${dns_records.type},中止`, '');
		$.done();
	}
	$.log(`${dns_records.type}類型內容:${dns_records.content}`, '');
	return dns_records
}
...
async function checkRecordInfo(zone, dns_records) {
	$.log('查詢記錄信息');
	if (dns_records.id) {
		$.log(`有記錄ID${dns_records.id}, 繼續`, '');
		var oldRecord = await getDNSRecord(zone, dns_records);
	} else if (dns_records.name) {
		$.log(`有記錄名稱${dns_records.name}, 繼續`, '');
		var oldRecord = await listDNSRecords(zone, dns_records);
	} else {
		$.log('未提供記錄ID和名稱, 終止', '');
		$.done();
	}
	$.log(`記錄查詢結果:`, `ID:${oldRecord.id}`, `名稱:${oldRecord.name}`, `類型:${oldRecord.type}`, `內容:${oldRecord.content}`, `代理狀態:${oldRecord.proxied}`, `TTL:${oldRecord.ttl}`, '');
	return oldRecord
}
...
async function setupRecord(zone, oldRecord, dns_records) {
	...
	$.log(`記錄更新結果:`, `ID:${newRecord.id}`, `名稱:${newRecord.name}`, `類型:${newRecord.type}`, `內容:${newRecord.content}`, `可代理:${newRecord.proxiable}`, `代理狀態:${newRecord.proxied}`, `TTL:${newRecord.ttl}`, `已鎖定:${newRecord.locked}`, '');
	return newRecord
}
	...
	return new Promise((resolve) => {
		$.get(url, (error, response, data) => {
			try {
				...
				$.logErr(`❗️${$.name}, ${getCFjson.name}執行失敗`, ` url = ${JSON.stringify(url)}`, ` error = ${error || e}`, `response = ${JSON.stringify(response)}`, `data = ${data}`, '')
			} finally {
				$.log(`🚧 ${$.name}, ${getCFjson.name}調試信息`, ` url = ${JSON.stringify(url)}`, `data = ${data}`, '')
				resolve()
			}
		})
	})
	...
	return new Promise((resolve) => {
		$.post(url, (error, response, data) => {
			try {
				...
				$.logErr(`❗️${$.name}, ${fatchCFjson.name}執行失敗`, ` url = ${JSON.stringify(url)}`, ` error = ${error || e}`, `response = ${JSON.stringify(response)}`, `data = ${data}`, '')
			} finally {
				$.log(`🚧 ${$.name}, ${fatchCFjson.name}調試信息`, ` url = ${JSON.stringify(url)}`, `data = ${data}`, '')
				resolve()
			}
		})
	})
