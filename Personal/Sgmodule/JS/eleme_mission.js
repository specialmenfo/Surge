...
// 以下條件是AND的關係
const taskKeywords = ["美食外賣"]; // 任務JSON中含有此關鍵字的任務才會領取
const requiredOrderAmount = 4; // 需要完成訂單數量小於等於此數的任務才會領取
...
function GetSuperVipMissions(cookie, longitude, latitude) {
  return new Promise((resolve, reject) => {
    ...
    magicJS.get(options, (err, resp, data) => {
      if (err) {
        magicJS.logError(`獲取會員任務失敗，請求異常：${err}`);
        reject("❌獲取會員任務失敗，請求異常，請查閱日誌！");
      } else {
        try {
          magicJS.logInfo(`獲取會員任務，接口響應：${data}`);
          ...
          if (obj) {
            ...
            magicJS.logWarning(`沒有可領取的會員任務，接口響應：${data}`);
            reject("❌沒有可領取的會員任務，請查閱日誌！");
          }
        } catch (err) {
          magicJS.logError(`獲取會員任務失敗，執行異常：${err}，接口響應：${data}`);
          reject("❌獲取會員任務失敗，執行異常，請查閱日誌！");
        }
      }
    });
  });
...
function AcceptMission(cookie, longitude, latitude, mission_id) {
  ...
  return new Promise((resolve, reject) => {
    ...
    magicJS.post(options, (err, resp, data) => {
      if (err) {
        magicJS.logError(`領取會員任務失敗，任務Id：${mission_id}，請求異常：${err}`);
        reject(`領取會員任務失敗，任務Id：${mission_id}`);
      } else {
        try {
          let obj = typeof data === "string" ? JSON.parse(data) : data;
          if (obj.success === true) {
            magicJS.logInfo(`領取會員任務成功，任務Id：${mission_id}，任務描述：${obj.mission.checkout_description}`);
            resolve(obj.mission.checkout_description);
          } else {
            magicJS.logError(`領取會員任務失敗，任務Id：${mission_id}，響應異常：${data}`);
            reject(`領取會員任務失敗，任務Id：${mission_id}`);
          }
        } catch (err) {
          magicJS.logError(`領取會員任務失敗，任務Id：${mission_id}，執行異常：${err}，接口響應：${data}`);
          reject(`領取會員任務失敗，任務Id：${mission_id}`);
        }
      }
    });
...
(async () => {
  ...
  if (!!!cookie) {
    ...
    if (getMissionErr) {
      ...
      magicJS.log("領取任務失敗，沒有發現符合要求的任務。請查閱任務返回JSON，確認是否因為任務描述改變而無法領取。");
      subTitle = "❌沒有符合要求的任務可以領取";
    } else {
      magicJS.logDebug(`獲取待領取的任務Id：${JSON.stringify(missions)}`);
      ...
      for (let i = 0; i < missions.length; i++) {
        ...
        magicJS.logInfo(`成功領取的任務Id：${JSON.stringify(acceptMissionList)}`);
      }
