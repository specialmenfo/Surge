...
// 存儲`全局備份` (棄用, 改用 `chavy_boxjs_backups`)
...
// 請求響應體 (返回至頁面的結果)
$.json = $.name // `接口`類請求的響應體
$.html = $.name // `頁面`類請求的響應體
...
async function handlePage() {
  ...
  // 如果沒有開啓調試模式，且當前版本與緩存版本一致，且直接取緩存
  if (!isDebugWeb && cache && cache.version === $.version) {
    $.html = cache.cache
  }
  // 如果開啓了調試模式，並指定了 `debugger_web` 則從指定的地址獲取頁面
  else {
  ...
  // 根據偏好設置, 替換首屏顏色 (如果是`auto`則交由頁面自適應)
  ...
  // 調試模式支持 vue Devtools (只有在同時開啓調試模式和指定了調試地址才生效)
...
async function handleQuery() {
  ...
  if (/^\/boxdata/.test(query)) {
    ...
    // TODO 記錄每次查詢的 key 至 usercfgs.viewkeys
...
function getBoxData() {
  ...
  // 把 `內置應用`和`訂閱應用` 里需要持久化屬性放到`datas`
  ...
  const sysapps = [
    {
      id: 'BoxSetting',
      name: '偏好設置',
      descs: ['可手動執行一些抹掉數據的腳本', '可設置明暗兩種主題下的主色調', '可設置壁紙清單'],
      keys: [
      ...
      settings: [
        { id: '@chavy_boxjs_userCfgs.httpapis', name: 'HTTP-API (Surge)', val: '', type: 'textarea', placeholder: ',examplekey@127.0.0.1:6166', autoGrow: true, rows: 2, persistentHint:true, desc: '示例: ,examplekey@127.0.0.1:6166! 注意: 以逗號開頭, 逗號分隔多個地址, 可加回車' },
        { id: '@chavy_boxjs_userCfgs.httpapi_timeout', name: 'HTTP-API Timeout (Surge)', val: 20, type: 'number', persistentHint:true, desc: '如果腳本作者指定了超時時間, 會優先使用腳本指定的超時時間.' },
        { id: '@chavy_boxjs_userCfgs.http_backend', name: 'HTTP Backend (Quantumult X)', val: '', type: 'text',placeholder: 'http://127.0.0.1:9999', persistentHint:true, desc: '示例: http://127.0.0.1:9999 ! 注意: 必須是以 http 開頭的完整路徑, 不能是 / 結尾' },
        { id: '@chavy_boxjs_userCfgs.bgimgs', name: '背景圖片清單', val: '無,\n跟隨系統,跟隨系統\nlight,http://api.btstu.cn/sjbz/zsy.php\ndark,https://uploadbeta.com/api/pictures/random\n妹子,http://api.btstu.cn/sjbz/zsy.php', type: 'textarea', placeholder: '無,{回車} 跟隨系統,跟隨系統{回車} light,圖片地址{回車} dark,圖片地址{回車} 妹子,圖片地址', persistentHint:true, autoGrow: true, rows: 2, desc: '逗號分隔名字和鏈接, 回車分隔多個地址' },
        { id: '@chavy_boxjs_userCfgs.bgimg', name: '背景圖片', val: '', type: 'text', placeholder: 'http://api.btstu.cn/sjbz/zsy.php', persistentHint:true, desc: '輸入背景圖標的在線鏈接' },
        { id: '@chavy_boxjs_userCfgs.changeBgImgEnterDefault', name: '手勢進入壁紙模式默認背景圖片', val: '', type: 'text', placeholder: '填寫上面背景圖片清單的值', persistentHint:true, desc: '' },
        { id: '@chavy_boxjs_userCfgs.changeBgImgOutDefault', name: '手勢退出壁紙模式默認背景圖片', val: '', type: 'text', placeholder: '填寫上面背景圖片清單的值', persistentHint:true, desc: '' },
...
function getUserApps() {
  // TODO 用戶可在 BoxJs 中自定義應用, 格式與應用訂閱一致
  return []
}
...
function upgradeUserData() {
  const usercfgs = getUserCfgs()
  // 如果存在`usercfgs.appsubCaches`則需要升級數據
 ...
 *
 * 升級前: 把所有備份都存到一個持久化空間
 * 升級後: 把每個備份都獨立存到一個空間, `$.KEY_backups` 僅記錄必要的數據索引
 */
