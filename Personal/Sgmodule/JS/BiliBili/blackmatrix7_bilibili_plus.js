    ...
    switch (true) {
      ...
      case /^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/tab/.test(magicJS.request.url):
        try {
          ...
          // 嘗試使用tab name直觀修改
          const tabNameList = new Set(["直播", "推薦", "熱門", "追番", "影視"]);
          ...
          magicJS.logError(`標籤頁處理出現異常：${err}`);
        }
        break;
      // 我的頁面處理，去除一些推廣按鈕
      case /^https?:\/\/app\.bilibili\.com\/x\/v2\/account\/mine/.test(magicJS.request.url):
        try {
          ...
          magicJS.logError(`我的頁面處理出現異常：${err}`);
        }
        break;
      // 直播去廣告
        ...
        try {
          ...
          obj.data.cards.forEach((element) => {
            if (element.hasOwnProperty("display") && element.card.indexOf("ad_ctx") <= 0) {
              // 解決number類型精度問題導致B站動態中圖片無法打開的問題
          ...
          magicJS.logError(`動態去廣告出現異常：${err}`);
        }
        break;
      // 去除統一設置的皮膚
