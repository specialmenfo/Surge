...
function GetCookie(){
  try{
    ...
    if (!!!hisCookie || compareCookie2 !== compareHisCookie2){
      ...
      magicJS.logInfo(`舊的Cookie：${hisCookie}\n新的Cookie：${cookie}\nCookie不同，寫入新的Cookie成功！`);
      magicJS.notify('Cookie寫入成功！！');
    }
    else{
      magicJS.logInfo('Cookie沒有變化，無需更新');
    }
  }
  catch(err){
    magicJS.notify('獲取Cookie出現異常，請查閱日誌。');
    magicJS.logError(`獲取Cookie出現執行異常，異常信息：${err}`);
  }
}
...
function GetPeaList(cookie, longitude, latitude){
  return new Promise((resolve, reject)=>{
    ...
    magicJS.get(options, (err, resp, data)=>{
      if (err){
        magicJS.logError(`獲取待領取的吃貨豆失敗，請求異常：${err}`);
        reject('獲取待領取的吃貨豆失敗，請求異常，請查閱日誌！');
      }
      else{
        try{
          magicJS.logDebug(`獲取待領取吃貨豆列表響應結果：${data}`);
          ...
          if (obj.success === true){
            ...
            magicJS.logInfo(`獲取待領取的吃貨豆成功：${JSON.stringify(peaList)}`);
            resolve(peaList);
          }
          else{
            magicJS.logError(`獲取待領取的吃貨豆失敗，響應異常：${data}`);
            reject('獲取待領取的吃貨豆失敗，響應異常，請查閱日誌！');
          }
        }
        catch(err){
          magicJS.logError(`獲取待領取的吃貨豆失敗，執行異常：${err}，接口響應：${data}`);
          reject('獲取待領取的吃貨豆失敗，執行異常，請查閱日誌！');
        }
      }
    })
  })
...
function DrawPea(cookie, peaId, longitude, latitude){
  return new Promise((resolve, reject)=>{
    ...
    magicJS.post(options, (err, resp, data)=>{
      if (err){
        magicJS.logError(`領取吃貨豆失敗，請求異常：${err}`);
        reject('領取吃貨豆失敗，請求異常，請查閱日誌！');
      }
      else{
        try{
          magicJS.logDebug(`領取吃貨豆響應結果：${data}`);
          let obj = typeof data === 'string'? JSON.parse(data) : data;
          if (obj.success === true){
            magicJS.logInfo(`領取吃貨豆成功：${data}`);
            resolve(true);
          }
          else{
            magicJS.logError(`領取吃貨豆失敗，響應異常：${data}`);
            reject('領取吃貨豆失敗，響應異常，請查閱日誌！');
          }
        }
        catch(err){
          magicJS.logError(`領取吃貨豆失敗，執行異常：${err}，接口響應：${data}`);
          reject('領取吃貨豆失敗，執行異常，請查閱日誌！');
        }
      }
    })
  })
...
;(async()=>{
  if (magicJS.isRequest){
    ...
    if (!!!cookie){
      magicJS.logWarning('沒有讀取到Cookie，請先從App中獲取一次Cookie！');
      content = '❓沒有讀取到有效Cookie，請先從App中獲取Cookie!!';
    }
    else{
      // 獲取待領取的吃貨豆
      ...
      if (getPeaListErr){
        ...
        for (let j=0;j<peaList.length;j++){
          ...
          if (drawPeaErr || result === false){
            drawPeaContent += `\n${peaList[j].description}-${peaList[j].count}吃貨豆-領取失敗`;
          }
          else{
            peaCount += peaList[j].count;
            drawPeaContent += `\n${peaList[j].description}-${peaList[j].count}吃貨豆-領取成功`;
          }
          await magicJS.sleep(1000);
        }
        content += `\n本次共領取吃貨豆${peaCount}個${drawPeaContent}`;
      }
    }
    // 通知
