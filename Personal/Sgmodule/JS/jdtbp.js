        ...
        if (body.data.apiStack) {
            ...
            try {
                ...
                if (tradeConsumerProtection) {
                    setTBItems(tradeConsumerProtection.tradeConsumerService.service.items, createTBItem("價格詳情", text));
...
function getBijiagoCookie(callback) {
        ...
        $.get(option, (err, rsp, data) => {
            ...
            set_tag_exp("cookie", cookie, 1000 * 60 * 60);
        ...
        $.get({ url: `${ScriptUrl}/config.json`, timeout: 3000 }, (err, resp, data) => {
            ...
            try {
                ...
                if (ScriptVersion !== obj.version)
                    $.msg(`腳本:${ScriptName} 發現新版本`, `版本號：${obj.version}`, `更新內容：${obj.msg}`);
            } catch (e) {
                $.logErr(e, resp);
            } finally {
                callback();
