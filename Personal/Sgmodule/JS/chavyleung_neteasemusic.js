...
function showmsg() {
  return new Promise((resove) => {
    $.subt = $.isWebSuc ? 'PC: 成功' : 'PC: 失敗'
    $.subt += $.isAppSuc ? ', APP: 成功' : ', APP: 失敗'
    if ($.isNewCookie && $.userInfo) {
      $.desc = `等級: ${$.userInfo.data.level}, 聽歌: ${$.userInfo.data.nowPlayCount} => ${$.userInfo.data.nextPlayCount} 升級 (首)`
      $.desc = $.userInfo.data.level === 10 ? `等級: ${$.userInfo.data.level}, 你的等級已爆表!` : $.desc
    }
    resove()
  })
}
