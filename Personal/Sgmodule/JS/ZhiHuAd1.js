...
if (url.indexOf("api.zhihu.com/commercial_api/real_time_launch_v2") !== -1 && method === getMethod) {
    ...
    if (dataArr === undefined) {
        ...
        body.data = dataArr.filter(item => {
            if (item.hasOwnProperty("extra") &&
                ...
                if (videoID == null) {
                    console.log('zvideo未獲取到videoID');
                    console.log("body:" + $response.body);
                    // 部分獲取不到videoId並且視頻可以播放
                    //$notification.post(notifiTitle, "知乎推薦列表視頻", "videoID獲取錯誤");
                } else {
                    console.log(`videoID處理成功,原始:${item.common_card.feed_content.video.id},修改為:${videoID}`);
                ...
                if (videoID == null) {
                    console.log("body:" + $response.body);
                    $notification.post(noticeTitle, "知乎推薦列表視頻", "videoID獲取錯誤");
                } else {
                    console.log(`market_card-videoID處理成功,原始:${item.fields.body.video.id},修改為:${videoID}`);
                ...
                console.log(`其他-videoID處理成功,原始:${item.common_card.feed_content.video.id},修改為:${videoID}`);
                item.common_card.feed_content.video.id = videoID;
            }
    ...
    console.log('知乎-問題回答列表');
    if (body.ad_info === undefined) {
        // 個別問題回答列表無廣告
        console.log("問題回答列表無廣告");
    } else {
        body.ad_info = null;
        console.log('成功');
    }
    body.data = body.data.filter(item => {
        if (item.hasOwnProperty("target_type") && item.target_type === 'answer'
            ...
            console.log(`feeds-video_id處理成功,原始:${item.target.attachment.video.video_info.video_id},修改為:${videoID}`);
            ...
            console.log(`v4-answers-video_id處理成功,原始:${item.attachment.video.video_info.video_id},修改為:${videoID}`);
            item.attachment.video.video_info.video_id = videoID;
        }
        return true;
    });
} else if (url.indexOf("www.zhihu.com/api/v4/answers") !== -1 && method === getMethod) {
    console.log('知乎-回答下的廣告');
    if (body.paging === undefined || body.data === undefined) {
        console.log("body:" + $response.body);
        $notification.post(noticeTitle, '知乎回答下廣告', "paging/data字段為undefined");
    } else {
        body.paging = null;
        body.data = null;
        console.log('成功');
    ...
    if (body.hasOwnProperty('config') && body.config.hasOwnProperty('zhcnh_thread_sync')
        ...
        if (body.config.zhcnh_thread_sync.ZHBackUpIP_Switch_Open === '1') {
            ...
            console.log('ZHBackUpIP_Switch_Open改為0');
        } else {
            console.log('無需更改ZHBackUpIP_Switch_Open');
        }
    } else {
        console.log("body:" + $response.body);
        $notification.post(noticeTitle, '知乎-appcloud2 config配置', "字段錯誤");
    }
} else if (url.indexOf("api.zhihu.com/commercial_api/app_float_layer") !== -1 && method === getMethod) {
    console.log('知乎-首頁右下角懸浮框');
    ...
    $notification.post(noticeTitle, "路徑/請求方法匹配錯誤:", method + "," + url);
}
...
function zhihuAds(body, name) {
    ...
    if (body.launch === undefined) {
        console.log("body:" + $response.body);
        $notification.post(noticeTitle, name, "launch字段為undefined");
    } else {
        launch = JSON.parse(body.launch);
    }
    if (launch.ads === undefined) {
        // ads字段有時候為空,有時候沒有ads字段
        console.log("body:" + $response.body);
        $notification.post(noticeTitle, name, "launch-ads字段為undefined");
    } else {
        launch.ads = [];
        console.log('成功');
    }
    ...
    if (i !== -1 && i !== url.length - 1) {
        ...
        for (let x = 0; x < arr.length; x++) {
            ...
            if (pair.length === 2) {
                ...
                console.log('url:' + url);
                $notification.post(noticeTitle, '獲取url參數', "pair錯誤");
            }
        }
    } else {
        console.log('url:' + url);
        $notification.post(noticeTitle, '獲取url參數', "i錯誤");
        return null;
    }
    // 未匹配到queryName
    return null;
