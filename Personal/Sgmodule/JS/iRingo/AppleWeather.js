...
if ($.getdata("iRingo")) {
	...
	$.log(`🚧 ${$.name}, BoxJs調試信息, iRingo類型: ${typeof iRingo}`, `iRingo內容: ${iRingo}`, "");
...
$.log(`🚧 ${$.name}, 調試信息, $.Apple.Weather類型: ${typeof $.Apple.Weather}`, `$.Apple.Weather內容: ${JSON.stringify($.Apple.Weather)}`, "");
	...
	return new Promise((resolve) => {
		...
		try {
			...
			if (api == 'v1' && weather.air_quality) {
				$.log(`⚠️ ${$.name}, ${getAQIstatus.name}檢測`, `AQ data ${api}, ${weather.air_quality?.metadata?.provider_name}`, '');
				var result = provider.includes(weather.air_quality.metadata.provider_name);
			} else if (api == 'v2' && weather.airQuality) {
				$.log(`⚠️ ${$.name}, ${getAQIstatus.name}檢測`, `AQ data ${api}, ${weather.airQuality?.metadata?.providerName}`, '');
				var result = provider.includes(weather.airQuality.metadata.providerName);
			} else {
				$.log(`🎉 ${$.name}, ${getAQIstatus.name}檢測`, "不存在 AQI data", '');
				var result = true;
			}
			*/
		} catch (e) {
			$.log(`❗️${$.name}, ${getAQIstatus.name}執行失敗`, `error = ${e}`, '');
		} finally {
...
function outputData(api, now, obs, body) {
	return new Promise((resolve) => {
		...
		try {
			//檢測版本
			$.log(`⚠️ ${$.name}, ${outputData.name}檢測`, `AQ data ${api}`, '');
			...
			//創建對象
			if (!weather[`${AQIname}`]) {
				$.log(`⚠️ ${$.name}, 沒有空氣質量數據, 創建`, '');
				weather[`${AQIname}`] = {
					"isSignificant": true, // 重要/置頂
					"pollutants": {},
					"metadata": {},
					"name": "AirQuality",
				};
				if (api == "v1") {
					...
					weather[`${AQIname}`].metadata.data_source = 0; //來自XX讀數 0:監測站 1:模型
				}
				else if (api == "v2") {
					weather[`${AQIname}`].metadata.units = "m";
					weather[`${AQIname}`].metadata.version = 2;
					weather[`${AQIname}`].sourceType = "station"; //station:監測站 modeled:模型
				}
			};
			// 注入數據
			//條件運算符 & 可選鏈操作符 
			...
			if (api == "v1") {
				...
				if (!weather.air_quality) {
					...
					weather.air_quality = {
						...
						"metadata": {
							"version": 1,
							"data_source": 0, //來自XX讀數 0:監測站 1:模型
						},
						"name": "AirQuality",
					};
				};
				...
				if (!weather.airQuality) {
					...
					weather.airQuality = {
						...
						"sourceType": "station", //station:監測站 modeled:模型
						"isSignificant": true, //重要/置頂
						"name": "AirQuality",
					}
				};
			...
			*/
			...
			$.log(`❗️${$.name}, ${outputData.name}執行失敗`, `瀏覽器訪問 https://api.waqi.info/api/feed/@${idx}/aqi.json 看看是不是空數據`, `原因：網絡不暢或者獲取太頻繁導致被封`, `error = ${error || e}`, '')
		} finally {
			// Output Data
				...
				if (error) throw new Error(error)
					...
					if (url.url.search("/nearest") != -1) {
						...
						if (url.url.search("/mapq/") != -1 && _data.d[0]) {
							...
							$.log(`❗️ ${$.name}, ${getNearestNOW.name}執行失敗`, `api: ${api}`, `data = ${data}`, '');
							$.done();
						}
						*/
						$.log(`🎉 ${$.name}, ${getNearestNOW.name}完成`, `idx: ${idx}`, `觀測站: ${name}`, `AQI: ${aqi}`, `距離: ${distance}`, '')		
						resolve([station, idx])
					}
					// Step 4
					// Get Nearest Observation Station Token
					// https://api.waqi.info/api/token/station.uid
						...
						if (_data.rxs.status == "ok") {
							...
							var token = "na";
							$.log(`⚠️ ${$.name}, ${getToken.name}執行失敗`, `status: ${_data.rxs.status}`, `data = ${data}`, '')
						} resolve(token)
						*/
					}
					// Step 5B
					...
					// https://api.waqi.info/feed/geo::lat;:lng/?token=:token
						...
						$.log(`🎉 ${$.name}, ${getCityFeed.name}完成`, `idx: ${city?.idx}`, `觀測站: ${city?.city?.name}`, `AQI: ${city?.aqi}`, '')
						resolve(city)
					}
					// Step 5C
					// Station Feed
					// https://api.waqi.info/feed/@station.uid/?token=:token
						...
						$.log(`🎉 ${$.name}, ${getStationFeed.name}完成`, `idx: ${station?.idx}`, `觀測站: ${station?.city?.name}`, `AQI: ${station?.aqi}`, '')
						resolve(station)
					}
				} else throw new Error(response);
			} catch (e) {
				$.logErr(`❗️${$.name}, ${getWAQIjson.name}執行失敗`, ` url = ${JSON.stringify(url)}`, ` error = ${error || e}`, `response = ${JSON.stringify(response)}`, `data = ${data}`, '')
			} finally {
				//$.log(`🚧 ${$.name}, ${getWAQIjson.name}調試信息`, ` url = ${JSON.stringify(url)}`, `data = ${data}`, '')
				resolve()
			}
		})
	})
	...
	return new Promise((resolve) => {
		$.post(url, (error, response, data) => {
			try {
						...
						if (_data.rxs.status == "ok") {
							if (_data.rxs.obs.some(o => o.status == 'ok')) {
								...
								if (i >= 0 && m >= 0) {
									...
									$.log(`❗️ ${$.name}, ${getStationAQI.name}`, `OBS Get Error`, `i = ${i}, m = ${m}`, `空數據，瀏覽器訪問 https://api.waqi.info/api/feed/@${idx}/aqi.json 查看獲取結果`, '')
									resolve(_data.rxs.obs[i].msg)
								}
				...
				$.logErr(`❗️${$.name}, ${fatchWAQIjson.name}執行失敗`, ` url = ${JSON.stringify(url)}`, ` error = ${error || e}`, `response = ${JSON.stringify(response)}`, `data = ${data}`, '')
			} finally {
				//$.log(`🚧 ${$.name}, ${fatchWAQIjson.name}調試信息`, ` url = ${JSON.stringify(url)}`, `data = ${data}`, '')
				resolve()
			}
		})
	})
