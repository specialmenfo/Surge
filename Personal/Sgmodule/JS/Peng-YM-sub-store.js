...
`
    ...
    async function downloadSubscription(req, res) {
        ...
        if (sub) {
            ...
            $.notify(`🌍 『 𝑺𝒖𝒃-𝑺𝒕𝒐𝒓𝒆 』 下載訂閱失敗`, `❌ 未找到訂閱：${name}！`);
            res.status(404).json({
                status: "failed",
            });
        }
    ...
    function createSubscription(req, res) {
        const sub = req.body;
        const allSubs = $.read(SUBS_KEY);
        $.info(`正在創建訂閱： ${sub.name}`);
        if (allSubs[sub.name]) {
            res.status(500).json({
                status: "failed",
                message: `訂閱${sub.name}已存在！`,
            });
        }
        // validate name
        if (/^[\w-_]*$/.test(sub.name)) {
            ...
            res.status(500).json({
                status: "failed",
                message: `訂閱名稱 ${sub.name} 中含有非法字符！名稱中只能包含英文字母、數字、下划線、橫槓。`,
            });
        }
    }
    ...
    function createCollection(req, res) {
        ...
        $.info(`正在創建組合訂閱：${collection.name}`);
        const allCol = $.read(COLLECTIONS_KEY);
        if (allCol[collection.name]) {
            res.status(500).json({
                status: "failed",
                message: `訂閱集${collection.name}已存在！`,
            });
        }
        // validate name
        if (/^[\w-_]*$/.test(collection.name)) {
            ...
            res.status(500).json({
                status: "failed",
                message: `訂閱集名稱 ${collection.name} 中含有非法字符！名稱中只能包含英文字母、數字、下划線、橫槓。`,
            });
        }
    }
    ...
    async function downloadRule(req, res) {
        ...
        $.info(`正在下載${builtin ? "內置" : ""}分流訂閱：${name}...`);
    ...
    function createArtifact(req, res) {
        ...
        $.info(`正在創建遠程配置：${artifact.name}`);
        const allArtifacts = $.read(ARTIFACTS_KEY);
        if (allArtifacts[artifact.name]) {
            res.status(500).json({
                status: "failed",
                message: `遠程配置${artifact.name}已存在！`,
            });
        } else {
            if (/^[\w-_.]*$/.test(artifact.name)) {
                ...
                res.status(500).json({
                    status: "failed",
                    message: `遠程配置名稱 ${artifact.name} 中含有非法字符！名稱中只能包含英文字母、數字、下划線、橫槓。`,
                });
            }
        }
    }
    ...
    function updateArtifact(req, res) {
        ...
        if (artifact) {
            ...
            if (
                ...
                res.status(500).json({
                    status: "failed",
                    message: `遠程配置名稱 ${newArtifact.name} 中含有非法字符！名稱中只能包含英文字母、數字、下划線、橫槓。`,
                });
            } else {
                const merged = {
                    ...artifact,
        ...
        for (const artifact of Object.values(allArtifacts)) {
            if (artifact.sync) {
                ...
                try {
                    ...
                    $.write(allArtifacts, ARTIFACTS_KEY);
                    $.info(`✅ 成功同步雲配置：${artifact.name}`);
                    success.push(artifact);
                } catch (err) {
                    $.error(`雲配置: ${artifact.name} 同步失敗！原因：${err}`);
                    $.notify(
                        `🌍 『 𝑺𝒖𝒃-𝑺𝒕𝒐𝒓𝒆 』 同步訂閱失敗`,
                        `❌ 無法同步訂閱：${artifact.name}！`,
                        `🤔 原因：${err}`
                    );
    ...
    async function deleteArtifact(req, res) {
        ...
        try {
            const artifact = allArtifacts[name];
            if (!artifact) throw new Error(`遠程配置：${name}不存在！`);
            if (artifact.updated) {
                // delete gist
                await syncArtifact({
                    filename: name,
    ...
    async function syncArtifact({filename, content}) {
        ...
        if (!gistToken) {
            return Promise.reject("未設置Gist Token！");
        }
    ...
    async function gistBackup(req, res) {
        ...
        if (!gistToken) {
            ...
            try {
                ...
                const msg = `${action === "upload" ? "上傳" : "下載"}備份失敗！${err}`;
                $.error(msg);
                res.status(500).json({
                    status: "failed",
                    message: msg,
    ...
    async function produceArtifact(
        ...
        if (type === "subscription") {
            ...
            proxies.forEach(p => {
                if (count[p.name]) {
                    $.notify("🌍 『 𝑺𝒖𝒃-𝑺𝒕𝒐𝒓𝒆 』", "⚠️ 訂閱包含重復節點！", "請仔細檢測配置！", {
            ...
            for (let i = 0; i < subs.length; i++) {
                ...
                $.info(
                    `正在處理子訂閱：${sub.name}，進度--${100 * ((i + 1) / subs.length).toFixed(1)
                    }% `
                );
                try {
                    const raw = await getResource(sub.url, useCache, sub.ua);
                    ...
                    $.error(
                        `處理組合訂閱中的子訂閱: ${sub.name}時出現錯誤：${err}! 該訂閱已被跳過。`
                    );
                }
            }
            if (!noProcessor) {
            ...
            for (let i = 0; i < rule.urls.length; i++) {
                ...
                $.info(
                    `正在處理URL：${url}，進度--${100 * ((i + 1) / rule.urls.length).toFixed(1)
                    }% `
                );
                try {
                    ...
                    $.error(
                        `處理分流訂閱中的URL: ${url}時出現錯誤：${err}! 該訂閱已被跳過。`
                    );
                }
            }
            // remove duplicates
    ...
    const PROXY_PROCESSORS = (function () {
        ...
        function getFlag(name) {
            ...
            const flags = {
                "🇦🇿": ["阿塞拜疆"],
                "🇦🇹": ["奥地利", "奥地利", "Austria", "维也纳"],
                "🇦🇺": ["AU", "Australia", "Sydney", "澳大利亚", "澳洲", "墨尔本", "悉尼", "土澳", "京澳", "广澳", "沪澳", "沪澳", "广澳"],
                "🇧🇪": ["BE", "比利时", "比利时"],
                "🇧🇬": ["保加利亚", "保加利亚", "Bulgaria"],
                "🇵🇰": ["巴基斯坦"],
                "🇰🇭": ["柬埔寨"],
                "🇺🇦": ["乌克兰", "乌克兰"],
                "🇭🇷": ["克罗地亚", "HR", "克罗地亚"],
                "🇨🇦": ["Canada", "CANADA", "CAN", "Waterloo", "加拿大", "蒙特利尔", "温哥华", "枫叶", "枫叶", "滑铁卢", "多伦多"],
                "🇨🇭": ["瑞士", "苏黎世", "Switzerland"],
                "🇳🇬": ["尼日利亚", "NG", "尼日利亚"],
                "🇨🇿": ["Czechia", "捷克"],
                "🇸🇰": ["斯洛伐克", "SK"],
                "🇷🇸": ["RS", "塞尔维亚"],
                "🇲🇩": ["摩尔多瓦", "MD", "摩尔多瓦"],
                "🇩🇪": ["DE", "German", "GERMAN", "德国", "德国", "法兰克福", "京德", "沪德", "广德", "沪德", "广德"],
                "🇩🇰": ["DK", "DNK", "丹麦", "丹麦"],
                "🇪🇸": ["ES", "西班牙", "Spain"],
                "🇪🇺": ["EU", "欧盟", "欧罗巴"],
                "🇫🇮": ["Finland", "芬兰", "芬兰", "赫尔辛基"],
                ...
                "🇮🇪": ["Ireland", "IRELAND", "爱尔兰", "爱尔兰", "都柏林"],
                "🇮🇱": ["Israel", "以色列"],
                "🇮🇳": ["India", "IND", "INDIA", "印度", "孟买", "Mumbai"],
                "🇮🇸": ["IS", "ISL", "冰岛", "冰岛"],
                "🇰🇵": ["KP", "朝鲜"],
                "🇰🇷": ["KR", "Korea", "KOR", "韩国", "首尔", "韩", "韩", "春川"],
                "🇱🇺": ["卢森堡"],
                "🇱🇻": ["Latvia", "Latvija", "拉脱维亚"],
                "🇲🇽️": ["MEX", "MX", "墨西哥"],
                "🇲🇾": ["MY", "Malaysia", "MALAYSIA", "马来西亚", "大马", "马来西亚", "吉隆坡"],
                "🇳🇱": ["NL", "Netherlands", "荷兰", "荷兰", "尼德兰", "阿姆斯特丹"],
                "🇵🇭": ["PH", "Philippines", "菲律宾", "菲律宾"],
                "🇷🇴": ["RO", "罗马尼亚"],
                "🇷🇺": ["RU", "Russia", "俄罗斯", "俄国", "俄罗斯", "伯力", "莫斯科", "圣彼得堡", "西伯利亚", "新西伯利亚", "京俄", "杭俄", "广俄", "沪俄", "广俄", "沪俄"],
                "🇸🇦": ["沙特", "迪拜"],
                "🇸🇪": ["SE", "Sweden"],
                "🇸🇬": ["SG", "Singapore", "SINGAPORE", "新加坡", "狮城", "沪新", "京新", "泉新", "穗新", "深新", "杭新", "广新", "广新", "沪新"],
                "🇹🇭": ["TH", "Thailand", "泰国", "泰国", "曼谷"],
                "🇹🇷": ["TR", "Turkey", "土耳其", "伊斯坦布尔"],
                ...
                "🇻🇳": ["VN", "越南", "胡志明市"],
                "🇮🇹": ["Italy", "IT", "Nachash", "意大利", "米兰", "义大利"],
                "🇿🇦": ["South Africa", "南非"],
                "🇦🇪": ["United Arab Emirates", "阿联酋"],
                "🇧🇷": ["BR", "Brazil", "巴西", "圣保罗"],
                "🇯🇵": ["JP", "Japan", "JAPAN", "日本", "东京", "大坂", "埼玉", "沪日", "穗日", "川日", "中日", "泉日", "杭日", "深日", "辽日", "广日"],
                "🇦🇷": ["AR", "阿根廷"],
                "🇳🇴": ["Norway", "挪威", "NO"],
                "🇨🇳": ["CN", "China", "回国", "中国", "中国", "江苏", "北京", "上海", "广州", "深圳", "杭州", "徐州", "青岛", "宁波", "镇江", "back"],
                "🇵🇱": ["PL", "POL", "波兰", "波兰"],
                "🇨🇱": ["智利"], "🇳🇿": ["新西兰", "新西兰"],
                "🇬🇷": ["希腊", "希腊"],
                "🇪🇬": ["埃及"],
                ...
                "🇦🇱": ["阿尔巴尼亚", "阿尔巴尼亚"],
                “🇪🇪”: [“爱沙尼亚”, "Estonia"],
                "🏳️‍🌈": ["流量", "時間", "應急", "過期", "Bandwidth", "expire"],
            };
...
function Gist({token, key}) {
    ...
    this.download = async function (filename) {
        const id = await locate();
        if (id === -1) {
            return Promise.reject("未找到Gist備份！");
        } else {
            try {
