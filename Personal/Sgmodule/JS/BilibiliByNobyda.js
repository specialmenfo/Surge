...
嗶哩嗶哩(白圖標外區版), 港澳台番劇自動切換地區 & 顯示豆瓣評分
...
****************************
[Script]
http-response ^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/(pgc\/view\/v\d\/app\/season|x\/offline\/version)\? script-path=https://raw.githubusercontent.com/NobyDa/Script/master/Surge/JS/Bili_Auto_Regions.js, requires-body=true, tag=bili自動地區

#可選, 適用於搜索指定地區的番劇
http-request ^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/x\/v\d\/search(\/type)?\?.+?%20(%E6%B8%AF|%E5%8F%B0|%E4%B8%AD)& script-path=https://raw.githubusercontent.com/NobyDa/Script/master/Surge/JS/Bili_Auto_Regions.js, tag=bili自動地區(搜索)
...
async function SwitchRegion(title, url, body) {
	...
	const HK = $.read('BiliArea_HK') || ‘🇭🇰 台灣節點'; //Your HongKong sub-policy name.
	...
	const area = (() => {
		...
		if (/\u6e2f[\u4e00-\u9fa5]+\u5340|%20%E6%B8%AF&/.test(title || url)) {
			...
			if (current != TW) select = { policy: TW, mode: '台灣' };
		} else if (body.code === -404 || chtMatch) {
			if (current != DF) select = { policy: DF, mode: '後備' };
		} else if (current != CN) {
			select = { policy: CN, mode: '直連' };
		}
...
async function QueryRating(body, play) {
	try {
		...
		if (!ratingEnabled && play.title && body.data && body.data.badge_info) {
			...
			body.data.badge_info.text = `⭐️ 豆瓣：${!$.is403 ? `${rating || ‘冇評'}分 (${folk || ‘冇評價'})` : `查詢頻繁！`}`;
