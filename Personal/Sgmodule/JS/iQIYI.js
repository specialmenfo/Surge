...
Node.js相關：
Cookie環境變量：IQIYI_COOKIE
Debug環境變量：IQIYI_DEBUG
Bark推送環境變量：BARK_PUSH

JsBox, Node.js用戶獲取Cookie說明：
方法一手機：開啓抓包, 網頁登錄 https://m.iqiyi.com/user.html 返回抓包APP搜索URL關鍵字 apis/user/info.action 複製請求頭中的Cookie字段填入以下腳本變量或環境變量中即可

方法二PC：網頁登錄 https://www.iqiyi.com 按F12控制台執行 console.log(document.cookie) 複製打印的Cookie填入以下腳本變量或環境變量中即可
*/

var cookie = ''; //單引號內填入手動獲取的Cookie
  ...
  if ($nobyda.isRequest) {
	...
	await WebCheckin();
        ...
        for (let i = 0; i < tasks.length; i++){
        	if (![1, 4].includes(tasks[i].status)) { //0：待領取 1：已完成 2：未開始 4：進行中
        ...
        if (!$nobyda.isNode) $nobyda.notify("愛奇藝", "到期時間: " + expires, pushMsg.join('\n'));
        if (barkKey) await BarkNotify($nobyda, barkKey, '愛奇藝', `到期時間: ${expires}\n${pushMsg.join('\n')}`);
        await $nobyda.time();
      } else {
        console.log(`Cookie缺少關鍵值，需重新獲取`)
      }
  } else {
...
function Checkin() {
  ...
  return new Promise(resolve => {
    ...
    $nobyda.post(URL, function(error, response, data) {
      ...
      try {
      	if (error) throw new Error(`接口請求出錯 ‼️`);
        const obj = JSON.parse(data)
        if (obj.code === "A00000") {
          if (obj.data.code === "A0000") {
            var quantity = obj.data.data.rewards[0].rewardCount;
            var continued = obj.data.data.signDays;
            CheckinMsg = `應用簽到: 獲得積分${quantity}, 累計簽到${continued}天 🎉`;
          } else {
            CheckinMsg = `應用簽到: ${obj.data.msg} ⚠️`;
          }
        } else {
          CheckinMsg = `應用簽到: Cookie無效 ⚠️`;
        }
      } catch (e) {
      	CheckinMsg = `應用簽到: ${e.message||e}`;
      }
      pushMsg.push(CheckinMsg);
      console.log(`愛奇藝-${CheckinMsg} ${Details}`);
      resolve()
    })
  })
}
      ...
      try {
        ...
        if (obj.code === "A00000") {
          if (obj.data[0].code === "A0000") {
            ...
            WebCheckinMsg = "網頁簽到: 獲得積分" + quantity + ", 累計簽到" + continued + "天 🎉"
          } else {
            WebCheckinMsg = "網頁簽到: " + obj.data[0].message + " ⚠️"
          }
        } else {
          WebCheckinMsg = `網頁簽到: ${obj.message||'未知錯誤'} ⚠️`
        }
    } catch (e) {
    	WebCheckinMsg = `網頁簽到: ${e.message || e}`;
    }
      pushMsg.push(WebCheckinMsg);
      console.log(`愛奇藝-${WebCheckinMsg} ${Details}`);
      resolve()
    })
  })
}
      ...
      $nobyda.get(URL, async function(error, response, data) {
        ...
        try {
          ...
          if (obj.awardName && obj.code == 0) {
            LotteryMsg = `應用抽獎: ${!$nobyda.last ? `${obj.awardName.replace(/《.+》/, "未中獎")} 🎉` : `您的抽獎次數已經用完 ⚠️`}`
          } else if (data.match(/\"errorReason\"/)) {
            const msg = data.match(/msg=.+?\)/) ? data.match(/msg=(.+?)\)/)[1].replace(/用戶(未登錄|不存在)/, "Cookie無效") : ""
            LotteryMsg = `應用抽獎: ${msg || `未知錯誤`} ⚠️`
          } else {
            LotteryMsg = `應用抽獎: ${data}`
          }
        } catch (e) {
        	LotteryMsg = `應用抽獎: ${e.message || e}`;
        }
        console.log(`愛奇藝-${LotteryMsg} (${s+1}) ${Details}`)
...
function getTaskList(task) {
  return new Promise(resolve => {
    $nobyda.get(`https://tc.vip.iqiyi.com/taskCenter/task/queryUserTask?P00001=${P00001}`, function(error, response, data) {
      ...
      console.log(`愛奇藝-任務列表: ${taskListMsg} ${Details}`)
      resolve(taskList)
    })
  })
}
  ...
  return new Promise(resolve => {
    $nobyda.get('https://tc.vip.iqiyi.com/taskCenter/task/joinTask?taskCode=' + task.taskCode + '&lang=zh_CN&platform=0000000000000000&P00001=' + P00001, function (error, response, data) {
    	...
    	try {
    		if (error) throw new Error(`請求失敗`);
    		const obj = JSON.parse(data);
    		joinTaskMsg = obj.code || '領取失敗';
    	} catch (e) {
    		joinTaskMsg = `錯誤 ${e.message||e}`;
    	}
    	console.log(`愛奇藝-領取任務: ${task.name} => ${joinTaskMsg} ${Details}`)
    	resolve()
    })
  })
}
  ...
  return new Promise(resolve => {
    $nobyda.get('https://tc.vip.iqiyi.com/taskCenter/task/notify?taskCode=' + task.taskCode + '&lang=zh_CN&platform=0000000000000000&P00001=' + P00001, function (error, response, data) {
    	...
    	try {
    		if (error) throw new Error(`請求失敗`);
    		const obj = JSON.parse(data);
    		notifyTaskMsg = obj.code || '失敗';
    	} catch (e) {
    		notifyTaskMsg = e.message || e;
    	}
    	console.log(`愛奇藝-開始任務: ${task.name} => ${notifyTaskMsg} ${Details}`)
    	resolve()
    })
  })
}
    ...
    $nobyda.get('https://tc.vip.iqiyi.com/taskCenter/task/getTaskRewards?taskCode=' + task.taskCode + '&lang=zh_CN&platform=0000000000000000&P00001=' + P00001, function (error, response, data) {
      ...
      try {
        ...
        if (obj.msg === "成功" && obj.code === "A00000" && obj.dataNew[0] !== undefined) {
          RewardsMsg = `任務獎勵: ${task.name} => ${obj.dataNew[0].name + obj.dataNew[0].value} 🎉`
        } else {
          RewardsMsg = `任務獎勵: ${task.name} => ${obj.msg!==`成功`&&obj.msg||`未完成`} ⚠️`
        }
      } catch (e) {
      	RewardsMsg = `任務獎勵: ${e.message||e}`;
      }
      pushMsg.push(RewardsMsg)
      console.log(`愛奇藝-${RewardsMsg} ${Details}`)
      resolve()
    })
  })
}
...
function GetCookie() {
  if (!$request.url.includes("/apis/user/info.action")) {
    $nobyda.notify(`寫入愛奇藝Cookie失敗`, "", "請更新腳本配置(URL正則/MITM)");
    return
  }
  ...
  if (CKA && iQIYI) {
    ...
    $nobyda.notify(`愛奇藝`, "", "寫入Cookie失敗，關鍵值缺失 ⚠️")
  }
}
