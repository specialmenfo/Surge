...
async function setupEnv(Verify, env) {
	...
	if (env.deviceType == "iOS") {
		...
		$.logErr('無可用設備類型', `deviceType=${env.deviceType}`, '');
		$.done();
	};
	//設置驗證方式
	if (Verify.Mode == "Token" && typeof Verify.Content != "undefined") {
		...
		$.logErr('無可用授權方式', `Mode=${Verify.Mode}`, `Content=${Verify.Content}`, '');
		$.done();
	};
}
...
async function WARP(setupMode, env, privateKey, publicKey, Verify) {
	try {
		$.log(`開始運行,模式:${setupMode}`, '');
		if (setupMode == "RegisterNewAccount") {
			if (!Verify.RegistrationId) {
				$.log('無設備ID(RegistrationId)', '');
				var result = await regAccount(env.Version, null, publicKey, env.Locale, env.deviceModel, env.Type, env.warp_enabled);
				$.log('生成完成,妥善保管以下四個憑證', `帳戶ID:${result.account.id}`, '賬戶ID:等同於匿名賬號', `許可證:${result.account.license}`, '許可證:可付費購買的訂閱，流量，邀請獎勵均綁定於許可證，一個許可證可以綁定5個設備(註冊ID)', `註冊ID:${result.id}`, '註冊ID:相當於WARP的客戶端或設備ID，配置信息均關聯到此註冊ID', `令牌:${result.token}`, '令牌:相當於密碼，更新讀取對應賬號所需，如果要更新註冊ID的配置或者更改關聯的許可證，需要此令牌驗證收發數據', '');
			} else {
				$.log(`不符合模式:${setupMode}運行要求，退出`, '');
				$.done();
			}
			...
			if (!Verify.RegistrationId && privateKey && publicKey) {
				...
				if (privateKey && publicKey) {
					$.log('有自定義私鑰(privateKey)', '有自定義公鑰(publicKey)', '');
			...
			if (Verify.License && Verify.RegistrationId) {
				$.log('有賬戶/許可證(License),有設備ID(RegistrationId)', '');
		...
		} else $.log(`未選擇運行模式或不符合模式:${setupMode}運行要求，退出`, `setupMode = ${setupMode}`, `License = ${Verify.License}`, `RegistrationId = ${Verify.RegistrationId}`, '');
	} catch (e) {
		$.logErr(e);
	} finally {
		return $.log(`${WARP.name}完成`, `result = ${JSON.stringify(result)}`, '');
		//return $.log(`${WARP.name}完成`, `名稱:${dns_records.name}`, `type:${dns_records.type}`, `content:${dns_records.content}`, '');
	}
};
