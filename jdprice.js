        ...
        if (body.data.apiStack) {
            ...
            try {
                ...
                if (tradeConsumerProtection) {
                    setTBItems(tradeConsumerProtection.tradeConsumerService.service.items, createTBItem("隞瑟聢霂行�", text));
                ...
                setTBItems(consumerProtection.items, createTBItem("隞瑟聢霂行�", text));
                if (consumerProtection.serviceProtection)
                    setTBItems(consumerProtection.serviceProtection.basicService.services, createTBItem("隞瑟聢霂行�", [text]));
...
function handleBijiago(data) {
    ...
    for (let promo_day of obj['analysis']['promo_days']) {
        ...
        if (promo_day['show'].indexOf("618隞瑟聢") != -1) {
            historyObj._618['price'] = price;
            historyObj._618['date'] = date;
        } else if (promo_day['show'].indexOf("��11隞瑟聢") != -1) {
        ...
        $.get({ url: `${ScriptUrl}/config.json`, timeout: 3000 }, (err, resp, data) => {
            ...
            try {
                ...
                if (ScriptVersion !== obj.version)
                    $.msg(`�𡁏𧋦:${ScriptName} �𤑳緵�啁��柄, `��𧋦�瘀�${obj.version}`, `�湔鰵��捆嚗�${obj.msg}`);
            } catch (e) {
                $.logErr(e, resp);
            } finally {
                callback();
